  ...
  const scrollToSection = useCallback((sectionId) => {
    ...
    setTimeout(() => {
      ...
      if (element) {
        ...
        console.warn(`Section with id "${sectionId}" not found. Please check if the element exists in the DOM.`);
        
        // Coba alternative scroll dengan querySelector
        const alternativeElement = document.querySelector(`#${sectionId}, [id="${sectionId}"]`);
              ...
              {NAV_ITEMS.map((item, index) => (
                <motion.button
                  ...
                  className={`relative text-gray-200 hover:text-violet-400 font-medium transition-colors duration-300 group bg-transparent border-none cursor-pointer ${
                    activeSection.toLowerCase() === item.id ? 'text-violet-400' : ''
                  }`}
                >
                  {item.name}
                  <span 
                    className={`absolute bottom-0 left-0 h-0.5 bg-violet-600 transition-all duration-300 ${
                      activeSection.toLowerCase() === item.id ? 'w-full' : 'w-0 group-hover:w-full'
                    }`}
                  />
                </motion.button>
              ))}
